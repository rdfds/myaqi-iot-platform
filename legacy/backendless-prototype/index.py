...
def sendData():
    ...
    if not deviceInfo == None:
              try:
                ...
                params = {'lat': deviceInfo[0]["latitude"], 'lon': deviceInfo[0]["longitude"], 'key': os.environ.get("IQAIR_API_KEY", "")}
                s = requests.Session()
...
def sendSMS(deviceSerialNumber, message):
    ...
    if (phoneList != ""):
        app.logger.info("Sending an AQI alert to configured contacts")
        ...
        if not all((account_sid, auth_token, twilio_phone_number)):
            app.logger.warning("Twilio configuration is incomplete; SMS alert skipped")
            return
...
def sendOutdoorData():
    ...
    payload = {}
    headers = {}
    params = {'lat': "39.7066", 'lon': "-73.5493", 'key': os.environ.get("IQAIR_API_KEY", "")}
